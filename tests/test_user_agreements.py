    ...
    response = client.post(
        ...
        json={
            "agreements": [
                ...
                {"type": "camera_microphone_notice", "agreed": True},
            ],
        },
    )
...
def test_post_partial_required_agreements_remains_incomplete(
    ...
    response = client.post(
        "/api/v1/users/me/agreements",
        headers=auth_headers(public_id, auth_settings),
        json={"agreements": [{"type": "terms_of_service", "agreed": True}]},
    )
    ...
    assert [item["agreed"] for item in body["agreements"]] == [True, False, False]
...
def test_post_same_agreement_updates_without_duplicate_rows(
    ...
    user_id, public_id = create_user(session_factory)
    payload = {"agreements": [{"type": "terms_of_service", "agreed": True}]}
    ...
    with session_factory() as db:
        row_count = db.scalar(
            select(func.count(UserAgreement.id)).where(
                ...
                UserAgreement.agreement_type == AgreementType.TERMS_OF_SERVICE,
            )
        )
        assert row_count == 1
