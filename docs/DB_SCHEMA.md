...
- `mediaDurationSeconds`: `answers.video_duration_seconds`. 컬럼이 INTEGER라 항상 정수 초로 보낸다(AI 개발자 문서 예시의 `39.9`처럼 소수 초는 안 보냄).
...
목적: Kakao 로그인 이후 Damso 자체 온보딩에서 필요한 필수 동의 4개 상태를 저장한다. Kakao Developers 동의항목과 별개다.

| Column         | Type           | PK  | FK       | Unique | Nullable | Notes                                                            |
| -------------- | -------------- | --- | -------- | ------ | -------- | ---------------------------------------------------------------- |
| id             | BIGINT         | Y   | N        | Y      | N        | 내부 PK                                                          |
| user_id        | BIGINT         | N   | users.id | N      | N        | 동의 사용자                                                      |
| agreement_type | agreement_type | N   | N        | N      | N        | `service_terms`, `privacy_policy`, `camera_microphone`, `data_usage` |
...
4개 항목이 모두 `agreed = true`일 때 온보딩의 필수 동의가 완료된 것으로 판단한다. 하나라도 동의하지 않으면 온보딩 완료로 처리하지 않는다. 선택 동의, 마케팅 동의, 동의 철회 기능은 MVP 범위에서 제외한다.
...
| invite_code        | VARCHAR(6)    | N   | N        | Y      | Y        | MVP 초대코드 정규화 저장값. 예: `A7K28Q` |
...
MVP에서는 활성 초대코드 1개를 `families.invite_code`에 저장한다. 저장값은 대문자/숫자 6자리 정규화 코드이며 하이픈을 포함하지 않는다. API 응답과 화면 표시에는 `XXX-XXX` 형식으로 변환하고, 입력값은 대문자 변환 및 하이픈/공백 제거 후 조회한다. 초대코드 이력, 만료, revoke가 필요해지면 `family_invite_codes` 테이블을 별도로 구현한다.
...
초대코드로 가족에 참여하면 `family_members.family_id`, `family_members.user_id`, `family_members.member_role`, `family_members.status = active`, `joined_at`을 저장한다. `member_role`은 참여 시점의 `users.role`을 그대로 사용한다. `(family_id, user_id)` unique index로 같은 사용자가 같은 가족에 중복 row로 생성되지 않게 하며, 서비스 계층은 활성 가족 소속 여부를 먼저 확인해 MVP의 "사용자는 하나의 활성 가족에만 속한다" 정책을 적용한다.
...
| agreement_type                 | `service_terms`, `privacy_policy`, `camera_microphone`, `data_usage` |
