...
`answerId`는 정수(예: `13`)뿐 아니라 숫자로 안 풀리는 문자열(예: `ans_001`)도 스키마 레벨에서는 받아들인다 — 다만 경로의 `answer_id`와 문자열 비교해서 다르면 `400`(`AnswerIdMismatchError`)이다. 또한 AI 서버가 `transcript`/`segments`/`pipelineResults`를 최상위가 아니라 `result.*`로 감싸서 보내는 경우가 실제로 관측되어(job 상태 조회 응답과 같은 형태), 최상위에 해당 필드가 없으면 `result` 안에서 찾아 보완한다.
...
- `Authorization: Bearer {callbackToken}`을 먼저 검증한다. `callbackToken`은 고정 audience(`aud: "ai-callback"`)와 `answer_id` 클레임을 담은 JWT로, 이 클레임이 경로의 `answer_id`와 일치하지 않거나 서명/만료가 유효하지 않으면 `401`을 반환한다(존재하지 않는 `answer_id`라도 토큰 검증을 먼저 하므로 `401`이 먼저 나갈 수 있다).
- 경로의 `answer_id`로 대상 `answers` row를 찾는다. 존재하지 않으면 `404`를 반환한다.
- body의 `answerId`가 경로의 `answer_id`와 다르면 `400`을 반환한다.
- 이미 `completed`/`failed`로 처리된 `answers`에 대한 재호출(AI 서버 재시도 등)은 재처리하지 않고 그대로 현재 상태를 담아 `200`을 반환한다(중복 broadcast 방지). 순차 재시도가 아니라 거의 동시에 두 콜백이 들어와 `video_clips` insert가 경합하는 경우에도, 나중에 실패한 쪽은 예외를 그대로 올리지 않고 이미 처리된 것으로 보고 `200`을 반환한다(unique 제약 위반이 `500`으로 새 나가지 않도록 처리).
...
{
  "answerId": 7,
  "questionText": "아버지가 가장 기억에 남는 가족 여행은 언제였어요?",
...
`videoUrl`, `thumbnailUrl`은 DB에는 GCS 경로만 저장돼 있고, 조회 시점에 signed GET URL로 변환해서 내려준다(HLS 변환 없이 mp4를 그대로 재생 — 답변 영상이 짧아 스트리밍 변환이 불필요하다고 판단). `videoDurationSeconds`는 `answers.video_duration_seconds` 컬럼값을 그대로 camelCase로 내려준다(변환 없음).

`questionText`는 `answers.question_send_id`로 연결된 `question_sends.question_text`를 그대로 내려준다(F-11 "오늘의 질문" 카드용).

`answererRole`/`answererName`은 `answers.user_id`로 연결된 `users.role`/`users.display_name`을 그대로 내려준다 — `answers.user_id`는 질문의 수신자(=답변 제출자)이므로 `question_sends`를 거치지 않아도 된다. 가족 다이어리 화면에서 답변자별로 클립을 구분해 보여줄 때 쓴다.
...
### `GET /api/v1/answers/{answer_id}/progress`

답변의 AI 처리 진행 상태를 조회한다. 처리 중 화면에서 진행률/현재 단계를 보여줄 때 사용한다. 접근 권한은 `GET /api/v1/answers/{answer_id}/clip`과 동일(현재 사용자의 활성 가족에 속한 답변만 조회 가능).

`answers.status`가 `processing`이 아니면(즉 아직 AI 서버에 디스패치되지 않았거나 이미 끝난 경우) AI 서버를 호출하지 않고 그 자리에서 바로 응답한다. `processing`인 경우에만 AI 서버의 `GET /api/v1/ai/jobs/{ai_job_id}?includeResult=false`를 직접 폴링해서 진행률 정보를 채워 내려준다.
...
`status`는 항상 이 백엔드의 `answers.status`이고, `aiJobStatus`는 AI 서버 자체 job 상태(`GET /api/v1/ai/jobs/{jobId}` 응답의 `status` 그대로)다. AI 서버가 처리를 끝내고도 콜백이 아직 도착하지 않은 구간에서는 `status="processing"`이면서 `aiJobStatus="completed"`일 수 있다 — 프론트는 이 조합으로 "처리 중"과 "완료 후 콜백 대기 중"을 구분해서 다른 문구를 보여줄 수 있다.

응답(처리 중이 아니거나 AI 서버 폴링 실패 — `progress`/`currentStepLabel`/`estimatedRemainingSeconds`/`aiJobStatus`는 전부 `null`):
...
AI 서버 폴링이 타임아웃/오류로 실패해도 `500`을 반환하지 않고 위처럼 진행률 필드만 비운 채 `200`으로 응답한다(클라이언트는 이전 진행률을 유지하거나 다음 폴링을 기다리면 된다).
...
`answererRole`/`answererName`은 `answers.user_id`(답변 제출자)로 연결된 `users.role`/`users.display_name`을 그대로 내려준다. 가족 다이어리(`/diary`) 화면에서 그리드를 답변자별로 구분해 보여줄 때 쓴다.
...
프론트엔드가 아니라 스케줄러(Cloud Scheduler 등)가 호출하는 운영용 엔드포인트. `Authorization: Bearer {INTERNAL_TRIGGER_TOKEN}`으로 인증한다(사용자 로그인 토큰과 무관한 별도 공유 비밀값).

### `POST /api/v1/internal/answers/reconcile`

콜백 유실 복구. `AiJobService.dispatch_job`은 fire-and-forget이라 AI 서버의 콜백이 어떤 이유로든(네트워크 실패, callbackUrl 오설정 등) 우리 쪽에 도달하지 못하면 `answers.status`가 `processing`에 무기한 머무른다. 이 엔드포인트는:

1. `status = processing`이고 `updated_at`이 `AI_STUCK_PROCESSING_THRESHOLD_MINUTES`(기본 20분)보다 오래된 `answers`를 찾는다.
2. 각각에 대해 AI 서버의 `GET /api/v1/ai/jobs/{ai_job_id}?includeResult=true`를 직접 폴링한다.
   - 응답이 `completed`/`failed`면 콜백 수신 때와 동일한 처리 로직(`AiCallbackService.apply_result`)을 그대로 재사용해서 `video_clips` insert/`answers.status` 갱신/Realtime broadcast까지 수행한다.
   - `404`(AI 서버에도 job 기록이 없음)면 복구 불가로 보고 `answers.status = failed`, `ai_retryable = true`로 처리한다(`AiCallbackService.mark_lost`).
   - 아직 `processing`/`queued`면 그대로 두고 다음 실행을 기다린다.
3. 이미 `completed`/`failed`로 처리된 `answers`는 `apply_result`/`mark_lost` 내부 idempotency 가드로 재처리하지 않는다 — 콜백이 폴링 직전/직후에 거의 동시에 도착해도 안전하다.
4. `status = submitted`이고 `updated_at`이 마찬가지로 임계값보다 오래된 `answers`도 찾는다 — AI 서버로의 최초 dispatch(`POST /api/v1/ai/jobs`)가 네트워크 실패 등으로 아예 실패하면 `processing`으로 못 넘어가고 `submitted`에 무기한 머무르는데, 이건 위 1번 쿼리로는 안 잡히기 때문이다. 이 경우엔 폴링 대신 `AiJobService.dispatch_job`을 그대로 재호출해서 dispatch 자체를 재시도한다.
...
`AI_SERVER_BASE_URL`이 설정되지 않은 환경에서는 아무 것도 하지 않고 전부 0인 응답을 반환한다.

에러:

- `INTERNAL_TRIGGER_TOKEN`이 서버에 설정되지 않음: `503`
- 토큰 누락/불일치: `401`
